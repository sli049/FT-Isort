...
_GLIBCXX_BEGIN_NAMESPACE_VERSION
  ...
  */
   ...
   *  @param  __first  Start of heap.
   *  @param  __last   End of heap + element.
   *ort @param  __comp   Comparison functor.
   *  @ingroup heap_algorithms
   *
  ...
  */
  ...
  template<typename _RandomAccessIterator, typename _Distance,
	   typename _Tp, typename _Compare>
    void
    __adjust_heap_err(_RandomAccessIterator __first, _Distance __holeIndex,
		  _Distance __len, _Tp __value, _Compare __comp)
    {
//	loop++;
      const _Distance __topIndex = __holeIndex;
	...
	//	     __first + (__secondChild - 1)))
	//    __secondChild--;
	    if (errnumber < 1 && loop%5000000 == 0){
//	    if (errnumber < 1){
		   errnumber++;
	//	std::cout<<"error injected."<<endl;
		std::cout<<errnumber <<endl;
	//	std::cout<<"left: "<< *(__first+(__secondChild-1))<<" "<<"right: "<<*(__first+__secondChild)<<". "<<"sifted down value: "<<__value<<endl;
		  if (__comp(__first + (__secondChild-1),
			     __first +__secondChild))
		    __secondChild--;
	//	std::cout<<"left: "<< *(__first+(__secondChild-1))<<" "<<"right: "<<*(__first+__secondChild)<<". "<<"sifted down value: "<<__value<<endl;
	//	if (*(__first + (__secondChild - 1)) < *(__first + __secondChild))
	//		__secondChild--;
	//	std::cout<<"secondChild index is: "<<(int) *(__secondChild)<<endl;
		}
	    else {
  ...
  template<typename _RandomAccessIterator, typename _Compare>
    ...
    //  std::__adjust_heap_err(__first, _DistanceType(0),
	//		 _DistanceType(__last - __first),
	//		 _GLIBCXX_MOVE(__value), __comp);
    }
